# -*- coding: utf-8 -*-
from data.data_loader_engine_trajectory_1 import DataReaderTrajactory, HILabeling #,Dataset_Pred_Trajectory
from models.model_1 import Informer, InformerStack
...
class Exp_Informer_1(Exp_Basic):
    def __init__(self, args):
        super(Exp_Informer_1, self).__init__(args) 
        self._HI_() 
    ...
    def test(self, setting):
        ...
        mae, mse, rmse, mape, mspe = metric(synthetics[:,:,:-1], trues[:,:,:-1])  #do not caculate the loss for HI column
        ...
        test_df = pd.read_csv('./data/FD001/test_normal_{}.csv'.format(self.args.HI_labeling_style),header=0)
        test_df.drop(['cycle'],inplace=True,axis=1)
        test_df.to_csv('./true_data.csv',header=True,index=False)
        synthetic_df['unit_id'] = test_df['unit_id']
        synthetic_data = synthetic_df.set_index(['unit_id'],drop=True,inplace=False)
        synthetic_data.to_csv('./synthetic_data_hi.csv',header=True,index=True) 
         
        
        synthetic_df.drop(['HI'],inplace=True,axis=1)
        synthetic_df['HI'] = test_df['HI']
        synthetic_df['unit_id'] = [i+100 for i in test_df['unit_id'].values]   
        synthetic_df.set_index(['unit_id'],drop=True,inplace=True)
        
        synthetic_df.to_csv('./data/FD001/Synthetic_data.csv',header=True,index=True)
        return
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y):
        ...
        batch_y = batch_y[:,-self.args.seq_len:,f_dim:].to(self.device)     
